...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        prior_state_level = self.s_levels[level] # Level before these actions
        actions = self.all_actions
        self.a_levels.append(set()) # New level
        for action in actions:
            action_node = PgNode_a(action)
            if action_node.prenodes.issubset(prior_state_level):
        # Connect to prior state node
                for state_node in prior_state_level:
    ...
    def add_literal_level(self, level):
        ...
        # Follow similar tasks to action level
        prior_action_level = self.a_levels[level - 1]
        self.s_levels.append(set())
        for action_node in prior_action_level:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Test to compare actions and preconds between nodes
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        not_same_symbol = node_s1.is_pos != node_s2.is_pos
        return same_symbol and not_same_symbol
        return False
